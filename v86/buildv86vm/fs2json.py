...
def handle_tar(logger, tar, use_compression):
    ...
    # Map normalised tar path → children list so we can look up parents in O(1).
    # The tar root ('.') is the filesystem root, so it maps directly to mainroot.
    dir_targets = {'.': mainroot, '': mainroot}

    for member in tar.getmembers():
        # Normalise: "./boot/" → "boot", "." → "" (root)
        normalized = member.name.strip('/')
        if normalized == '.':
            normalized = ''

        # Skip the root directory entry itself — mainroot already represents it.
        if normalized == '':
            continue
        ...
        dir = dir_targets.get(parent_key, mainroot)
        ...
        if member.isfile() or member.islnk():
            ...
            children = []
            obj[IDX_TARGET] = children
            dir_targets[normalized] = children
        elif member.issym():
